-- Grant portal:navigation:write wherever entity_builder:schema:tenant_write is already granted (same tenant role rows).
INSERT INTO role_permissions (id, tenant_id, role_id, permission_id)
SELECT gen_random_uuid(), rp.tenant_id, rp.role_id, pn.id
FROM role_permissions rp
JOIN permissions tw ON tw.id = rp.permission_id AND tw.code = 'entity_builder:schema:tenant_write'
