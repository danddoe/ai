            ...
            if (StringUtils.hasText(token)) {
                ...
                if (!claims.isRefresh()) {
                    List<SimpleGrantedAuthority> authorities = Stream.concat(
                            ...
                            claims.getPermissions().stream().map(p -> new SimpleGrantedAuthority(p))
                    ).collect(Collectors.toList());
                    TenantPrincipal principal = new TenantPrincipal(claims.getUserId(), claims.getTenantId(), claims.getEmail(), claims.getRoles(), claims.getPermissions());
